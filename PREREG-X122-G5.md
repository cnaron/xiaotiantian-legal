...
| 元素 | **同型元素的条数增减**(`<li>`↔`<li>`、`<tr>`↔`<tr>`、`<p>`↔`<p>`),且只用页面里**已经出现过**的标签与 class | 新标签类型、新 class、新属性、改 `<style>`、改 CSS、改字体、加图片/脚本 |
...
3. **privacy §一.1 个人信息**:F11(「联系我们」是唯一网络请求,会带走 8 个字段)**必须写进隐私政策**,
   而原版这一节只有 1 个 `<p>`,全塞进去会变成一个没人读得完的长段。⇒ **拆成 2 个 `<p>`**(同型)。

## 2.2 本轮**元素条数变化的完整清单**(在改动前登记死,回执逐条对账)

| 页 | 位置 | 变化 | 理由 |
|---|---|---|---|
| privacy | §一.1 | `<p>` 1 → **2**(+1) | F11 唯一网络请求,单段塞不下 |
| privacy | §一.3 权限 `<ul>` | `<li>` 2 → **6**(+4) | F1 权限 6 条,审核逐条对 |
| terms | §三.1 价格 `<tbody>` | `<tr>` 2 → **5**(+3) | F14 五档价格 |
| index | 「关于本站」 | `<section>` 1 → **0**(−1) | owner 追加令② 技术/托管性说明 |
| 其余全部位置 | — | **零变化** | 只换字 |

**合计:+1 `<p>`、+4 `<li>`、+3 `<tr>`、−1 `<section>`,除此之外一个元素都不增减。**
超出这张表的任何增减都算偏离,回执必须点名。
